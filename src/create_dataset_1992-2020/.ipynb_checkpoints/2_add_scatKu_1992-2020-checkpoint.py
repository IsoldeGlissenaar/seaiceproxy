...
for i in range(len(years_os)):
    for j in range(len(days)):
        ...
        try:
            ...
            sir_fname = direc+'/Scatterometer/OSCAT/'+str(yr)+'/grd/'+f
...
for i in range(len(years_ss)):
    for j in range(len(days)):
        ...
        try:
            ...
            sir_fname = direc+'/Scatterometer/ScatSat-1/'+f
...
for i in range(len(years_qs)):
    for j in range(len(days)):
        ...
        try:
            ...
            sir_fname = direc+'/Scatterometer/QuickSCAT/grd/'+f
            image, head, descrip, iaopt = loadsir(sir_fname)
        except FileNotFoundError:
            try:
                f = 'queh-a-Arc'+str(yr)[2:]+'-'+str("{:03d}".format(int(jd-0.5)))+'-'+str("{:03d}".format(int(jd-0.5)))+'.grd'
...
dataset_X.to_csv('../../data/dataset/Ku-band/'+location+'_dataset_x_1992-2019_'+str("{:02d}".format(m))+'.csv',
                    index=False)
dataset_y.to_csv('../../data/dataset/Ku-band/'+location+'_dataset_y_1992-2019_'+str("{:02d}".format(m))+'.csv',
                    index=False)
